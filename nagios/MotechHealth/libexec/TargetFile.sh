		...
		if [ $i -le 1 ]
    		then
				...
				#Query to get state name against a circle
                                state_name=$(ssh -p 1027 grameen@192.168.200.5 'mysql -N -e "use motech_data_services;select name from nms_states where id IN (select distinct(state_id_OID) from nms_districts where circle_id_OID = (select id from nms_circles where name='"'${word[1]}'"'));"') 
